...
> **Revised 2026-08-07 (owner)** together with PRD v2 §8.4: no pre-anchoring, two taps per scene, and the story keeps playing while the cut is decided. Rationale in `docs/segmentation-rules.md`.
**Keep — this interaction is validated, replicate it exactly:**
- Scenes are cut **sequentially from a locked frontier**, which is a **floor, not an anchor**: the user taps where the scene **begins** and then where it **ends** (PRD v2 §8.4).
...
- A selection band highlights the range; a big **"✓ Confirmar esta cena"** bar sits under the player. Confirming locks the scene, marks its end bead square, and immediately opens the next scene, **empty**.
