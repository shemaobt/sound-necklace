...
- Scenes are cut **sequentially from a locked frontier**. **Amended (owner, 2026-08-07):** the start is no longer pre-anchored — the frontier is a floor, and the user taps where the scene **begins** and then where it **ends** (PRD v2 §8.4). `primePart()` no longer exists.
...
- A selection band highlights the range; a big **"✓ Confirmar esta cena"** bar sits under the player. Confirming locks the scene, marks its end bead square, and immediately opens the next scene — **empty** since 2026-08-07, not pre-anchored at the seam.
...
**Decided:** cream stage; the instruction names both taps in one line — *"Toque no colar onde esta cena **começa e termina**."* — with "começa e termina" emphasized in telha, and it does **not** change as the taps happen (amended 2026-08-07: the earlier copy, *"onde esta cena termina. O começo já está costurado."*, described a pre-anchored start that no longer exists, and switching the text mid-tap put text in competition with sound). With ≥ 1 scene locked the second sentence becomes *"Toque numa cena pronta para reouvir."*, verbatim from the "Ouvir no colar" study (§11). Confirmed scenes render as compact pill **chips** (swatch · Cena N · Reabrir — inert body, no ▶: the beads play the scene) in a wrapping row; the dark **"Confirmar as cenas →"** button appears only once ≥ 1 scene is locked. The seam advancing to the next scene should animate.
...
- **`novos componentes.html`** ("Peças novas") — the components designed for the Mapeamento/conversation flow: the **autosave chip** (*Salvando…* ⇄ *Tudo salvo — pode sair e voltar*, two states; offline/retry stays *Salvando…*, never claims saved while pending — ENG-351); the two progress indicators — **progresso geral**, the six-step stepper (Ouvir · Cortar · Triagem · Frases · Conversa · Guardar) realigned to rounded telha rectangles, **superseding the dot treatment** carried over from `Protótipo` (§5.1 — ENG-353); and **progresso da conversa**, a continuous per-*trecho* bar (história / cenas / frases, each in its colour, subtle dividers at boundaries, a position marker — ENG-350); and the **indicador + reprodutor do trecho** (colour dot + trecho name beside a play/pause of that trecho's audio — ENG-350). Note: the listener-facing rule (no counters/numbers/IDs) **overrides** this prototype's numeric labels — drop "Trecho X de N" and "Cena 1", keep colour + scene-kind label.
