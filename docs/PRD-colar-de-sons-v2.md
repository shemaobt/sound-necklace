...
- Scenes: the frontier is `max(locked scene end) + 1` (floor 0, capped at the last bead). **Amended (owner, 2026-08-07):** a new scene is **no longer pre-anchored** — the frontier is now only a FLOOR (a click before it saturates at it), and the start comes from the user's first click. Cutting a scene takes two taps: where it begins, then where it ends. Consequence the owner accepted: an *accidental* gap between scenes became possible, where contiguity used to be structural.
...
- **Hover edge preview (mouse only; field-requested):** dwelling ~280 ms within ±1 bead of a selection edge plays that boundary without changing it; re-arms after the pointer leaves the edge's proximity. **Amended (owner, 2026-08-07): the preview only fires in SILENCE.** With sound in progress the sound wins, whatever it is — the dwell had been cutting off the audio a tap had just started, three times over. A paused playback counts as silence. This is a rule of precedence, not of pointer position.
...
- **Amended (owner, 2026-08-07).** A new scene opens **empty**, and the cut takes two taps. The audio is what guides the decision:
...
- **"✓ Confirmar esta cena":** requires a completed selection ("Clique onde a cena termina, no colar.") starting at/after the frontier ("A cena não pode começar antes da conta X."). On success the scene locks, its end bead turns square, and the next scene opens **empty** (2026-08-07 — it used to open pre-anchored at the seam).
...
- **Phrase anchoring** follows the frontier logic (§6.4), including the first-phrase back-reach. **Amended (owner, 2026-08-07):** the phrase now takes the **same two taps as the scene** (§8.4) — begin, then end, with the SCENE playing on from the first tap. The reference is asymmetric here (only scenes were pre-anchored); we are not, deliberately: scene and phrase are one model. Instruction copy: *"Toque no colar onde esta frase começa e termina."* Validation copy: "Clique onde a frase termina, no colar." / "A frase não pode começar antes da conta X." / "A frase precisa terminar dentro do colar."
...
3. **Never re-listen to the whole segment when fine-tuning.** When adjusting a scene or phrase boundary, the user is never forced to hear the segment again from the top. **Amended (owner, 2026-08-07):** what plays is **the stretch that changed hands** — from the boundary's old position to its new one — rather than the margins around it. Hearing where the boundary *is* does not tell you what the move *did*; the stretch that moved does. The margins remain the mechanic for the hover lens (§8.2) and the drag preview. The affordance must be *legible without instruction text* (emphasized edge beads on the selection band).
...
| Scene start | user's first tap (2026-08-07); the frontier is a floor, not an anchor |
...
| Hover edge preview | mouse only; ±1 bead of an edge; ~280 ms dwell; **silence only** (2026-08-07) |
