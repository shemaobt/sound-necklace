...
The audio is sliced into a fixed grid of tiny **contas** (beads), whose uniform duration comes from the project's granularity level (§6.1). Every boundary snaps to a bead index; bead indices are the universal coordinate system of the entire pipeline. The output is a set of artifacts (two JSON documents + a Markdown report) consumed downstream by an automated pipeline ("o Compilador" / Claude Code agents).
...
- **Acoustemes** are the data responsible for audio granularity. They accompany each audio retrieved from the project bucket (§7.4). For bucket audios, each granularity level maps to a uniform bead duration **derived from that audio's acoustemes** (derivation rule: §15.1 O8). For audios lacking acousteme data (an edge case in the MVP, where all audios come from the bucket; the normal case for post-MVP uploads), the levels map to fixed fallback durations: **0.20 / 0.50 / 1.00 s** for small / medium / large (the tokenizer's own grid — 20 ms hop × 10/25/50 frames; §15.1 O8). The v1 prototype's 0.25 s was the reference before O8 resolved and is no longer a value anything uses.
...
**Confirming every draft at once (decided 2026-08-12 — owner).** The report carries a **bulk confirmation**: one action that turns every pending transcription draft into its answer, behind a dialog that names what is being accepted (machine-written texts, becoming the answers, without a per-answer reading). It exists because confirming a real session's hundreds of recorded answers one at a time is not a workflow. It is **the same act as the single confirm, repeated** — same reserved keys, same PT/EN asymmetry — applied in one state transaction, and it is **eligible only where an answer has a draft and its cell is still empty**, so a facilitator's hand-typed answer is never overwritten. It reports what it did — how many were confirmed, and how many recorded answers still have no transcript and need to be written by hand. The export gate is untouched: this **satisfies** it, and nothing bypasses it. The policy consequence — that a human now attests to N drafts in one act rather than reading each — is recorded in §12.
...
- **Speech-to-text of the voice answers (decided — and its boundary):** an answer recording may be transcribed into an **editable draft** (§8.7). It runs **through our own API** — the SPA never calls a transcription provider — and the transcript is **advisory data with no authority of its own**: a human confirms it, and until then it is not an answer, does not appear in an artifact, and never overwrites the recording, which remains the record of what the listener said. Sending answer audio out for transcription is the *only* case where listener audio leaves our systems; it goes to a processor bound to no-training and zero-retention, under the LGPD basis and consent recorded above, and it is **disclosed on the setup screen**. The **story audio is never sent** — it stays inside our control under every circumstance.
  - **Amended (owner, 2026-08-12): the confirmation may be given in bulk.** The report offers a single action that confirms **every pending transcription draft at once**, behind a dialog that says plainly what is being accepted. This does not remove the human from the rule — a person still decides, deliberately, that these texts become the answers — but it changes what that person is attesting to: **one act accepting N drafts, instead of N readings**. That is the facilitator's call to make, and the reason it exists is concrete: a long interview reaches the report with hundreds of recorded answers, and confirming them one at a time is not a workflow. The boundaries around it are unchanged and non-negotiable: the export gate (`reportExportStatus`) is **satisfied by confirming and by nothing else** — there is no override, no force flag, no second click that pushes past it; the bulk action only ever touches an answer that **has a draft and whose answer cell is still empty**, so it can never overwrite text a human typed; and an unconfirmed draft still never reaches an artifact.
...
| O8 | Acousteme granularity mapping *(2026-07, ENG-242 / tripod-api PR #100)* | `beadSec = granularity_frames[level] × hop_sec` off the audio's own envelope; for audios without acoustemes, the tokenizer's fixed grid (20 ms hop, presets 10/25/50 frames → **0.20 / 0.50 / 1.00 s**). The remaining obligation is on the pipeline, not this SPA: acousteme params must be **uniform per project**, so every audio of a project resolves to the same `beadSec` (ENG-352) — the SPA refuses a divergent audio rather than cutting it on a second grid (§6.1). |
...
| O11 | Bulk confirmation of transcription drafts *(2026-08-12, owner)* | The report may confirm **all** pending transcription drafts in **one deliberate act**, behind a dialog. The human stays in the rule but attests to N drafts at once instead of reading each — the facilitator's call, taken because a session with hundreds of recorded answers cannot be confirmed one click at a time. The export gate `reportExportStatus` is **untouched**: bulk confirmation *satisfies* it; there is no override or force flag. Eligible only where a draft exists **and** the answer cell is empty, so hand-typed answers are never overwritten (§8.7, §12). |
