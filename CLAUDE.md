...
Colar de Sons is an ear-first web app where a facilitator + a listener from an oral culture segment a recorded oral story (necklace of audio beads) into scenes and phrases, and classify those scenes against the Ruth ontology, then look at the whole story together and conclude it (the **Rever** station, ENG-725, owner decision 2026-09-02). **The app ends there** (owner decision, 2026-09-01, ENG-689/ENG-691): the segmentation is kept by the autosave, and the SPA produces no artifact and hands nothing to a downstream pipeline. The interview by voice, the report, the three exported artifacts and "o Compilador" were the product's other half until that decision; they come back later, in another flow, in another product. This repo is a **complete from-scratch implementation** — no code is reused from the v1 prototype.
...
- `ui/` — Shemá design system + the flow stations, organized by **atomic design**:
  ...
  - `ui/templates/` + `ui/pages/` — station layouts and the wired screens (Setup, Escuta 1/2, Triagem, Segmentação, Rever, Dashboard, Login).
...
- Gates: whole story → scenes → (all triaged + ≥1 productive) → segmentação → **end**. All-none-fit locks downstream. Confirming the last productive scene's phrasing puts the session in `mode: 'concluida'`, which for the domain is the end of the flow and NOT a station (ENG-691) — `modeLocks` has no key past `segmentacao`. The UI mounts the Rever station on that mode (ENG-725); the domain does not know it.

## UI rules

- **The app runs Ouvir → Cortar → Triagem → Frases → Rever, and ends there** (owner decisions: the cut on 2026-09-01 — ENG-689 scope cut 1/4, ENG-691 scope cut 2/4 — and the Rever on 2026-09-02, ENG-725). Confirming the last productive scene lands the pair on the Rever — the whole story at a glance, nothing editable there — and closing the session is an explicit act, "Concluir a história", which asks for a second tap while any scene is in doubt or without a kind. There is no conversation, no report, no export, and **the SPA generates no artifact**. The cuts, scenes and phrases are kept by the autosave, for a different system to consume. `confirmFrasesDone` returns `kind: 'finished'` and `domain/` moves to `mode: 'concluida'`, which is what mounts the Rever; there is no station after it. The interview comes back later, in another flow, in another product. **Slices 3–4 of the cut still have to land**, and the session is not yet marked concluded on the server (ENG-702).
