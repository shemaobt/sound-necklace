...
- **Port** (@/adapters/stt/types.ts): `start(sessionId, paths, { force })` and `progress(sessionId) → { done, drafts }`, with `drafts` a `path → { source, en, generation }` map. `source` is the transcript in the spoken language, and it is what the facilitator confirms; `en` is the English derived from it, which the artifact emits. Drafts arrive **all at once at the end**, not incrementally — a half-filled map is never observable.
- **`generation` is the draft's freshness key**, and the port carries it because nothing else can answer the question the report asks. A re-transcription on the server does not touch the recording, so the recording's version reads "unchanged" even when the text was replaced outright — which is exactly how a session transcribed before the disfluency cleanup kept exporting the stutters after a successful re-run. It is **required, never optional**: an absent field invites a local `0`, and `0` is a confirm the server refuses with no way back.
...
- **`confirm(sessionId, path, transcript, generation)`** takes the corrected transcript back and returns `{ en, generation }`. It exists because the report reads the English field: an edit applied only in the SPA would leave the artifact carrying the translation of the sentence it replaced. Only the **spoken-language text** goes up — the English is re-derived server-side and is never the client's to send. The `generation` makes it a compare-and-swap; a superseded one rejects with **`TranscriptSuperseded`**, which is its own error type because it demands the opposite reaction to an ordinary failure: retrying replays the same stale counter forever, and only re-reading the draft helps. Confirming text identical to what the server holds returns immediately without translating, so re-confirming is free and the bulk paths can lean on it.
...
- **`HttpTranscriber`** (@/adapters/stt/http.ts): the real port over `POST`/`GET /api/sound-necklace/sessions/{id}/transcriptions`. `start` POSTs `{ language, force }` (the server derives the recorded paths from the session, so the port's `paths` are unused here); `progress` GETs `{ total, ready, failed, pending, answers[] }`, maps `done = pending === 0`, and turns each **ready** answer into `{ source: transcript_source, en: translation_en, generation }` — pending/failed answers carry no English and are simply absent from the draft map. `language` is the interview's BCP-47 locale, required by the server and injected from the UI language by the wiring. The JSON is validated by an **adapter-local Zod schema** (not a `contracts/` DTO): these `/api/sound-necklace/` routes could be pulled into the OpenAPI snapshot, but keeping the shape local avoids a frozen-layer change and a dependency on production being deployed. `progress` **rejects** on a non-ok response so the hook retries instead of reading garbage as done; a 401 pings `onUnauthorized`.
