...
- **`types.ts`** — the two ports plus typed errors: `AuthError` (bad credentials / no session — the UI shows copy) and `ApiError` (non-2xx, carrying `status` + the parsed error `body`). `Credentials`/`AuthUser` reuse the contract DTOs. `AuthUser.roles` is what was _granted_ and nothing else: a platform admin gets a session with an empty list (see `hasAppAccess` below) and no synthetic role is invented to paper over it. The port deliberately carries no admin flag — the two places that need one read it off the wire response they were already fetching.
...
- **The client-side access gate is cosmetic.** `hasAppAccess` only decides whether the SPA renders a session instead of bouncing to "sem acesso ao Colar de Sons" — the tripod-api is what actually enforces authorization on every request; a platform admin let through here who somehow lacked real API access would simply fail on the first call, same as anyone else.
