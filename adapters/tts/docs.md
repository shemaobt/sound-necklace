...
- **`WebSpeechSynthesizer`** (@/adapters/tts/web.ts, now the FALLBACK): over the Web Speech API. Sets `utterance.lang`, cancels prior speech before each `speak`, reflects `start`/`end`/`error` into the speaking state. Still exports `speechSynthesisSupported()`, though `register.ts` no longer gates on it.
...
- **A stale clip must never play.** Each `speak()` takes a generation number; a response that lands after the listener has already advanced is discarded. Without this, arriving at question 5 could be interrupted by question 4's late audio.
