...
- **`HttpSessionStore`** (@/adapters/sessions/http.ts) — real skeleton mapping each port method to a tripod-api endpoint (ENG-262) with an **injected** `fetch` (no network in CI) and a Bearer-token getter. JSON responses that are contract-shaped (`SessionSummary`, list, `LockStatus`) are schema-parsed; the opaque state/artifacts and the WebM resource bytes travel without re-serialization. `acquireLock` and `renewLock` both `PUT /sessions/{id}/lock` — the backend serves acquire and renew as the same idempotent operation, and there is no POST on that route (it would 405). Endpoints are **provisional** until the OpenAPI lands (ENG-211/ENG-247).
...
- **A write can still be rejected — and that rejection is a verdict, not a retry-me.** `HttpSessionStore`'s autosave `persist` catches a 409 on the state PUT (fencing by `user_id`), reports it once via the optional `onLockLost(id)` callback (autosave is fire-and-forget, so a callback is the only way the UI learns) and rethrows a `LockLostError`. `createAutosaver` treats that error as terminal — no retry, and the pending state is dropped instead of held in memory, which would otherwise lose the write silently when the tab closes. A transient 5xx still retries with backoff, unaffected. `complete`/`reopen` skip that translation — a 409 there reaches the caller as the raw `ApiError` (@/adapters/api, status 409) instead of `LockLostError`. The fixture never fences writes at all, so this path only exercises against `HttpSessionStore`.
