...
describe('AcoustemeGranularityResolver.resolve — regra O8 (frames × hop)', () => {
  it('deriva do envelope: 10/25/50 frames × 20 ms = 0.20 / 0.50 / 1.00 s', () => {
    ...
    expect(r.resolve('pequena', env).beadSec).toBe(0.2);
    expect(r.resolve('media', env).beadSec).toBe(0.5);
    expect(r.resolve('grande', env).beadSec).toBe(1);
  });
  ...
  it('mapeia cada nível à sua chave da grade (small/medium/large), lendo o envelope', () => {
    ...
    expect(r.resolve('pequena', env).beadSec).toBe(0.5); // 25 × 0.02
    expect(r.resolve('media', env).beadSec).toBe(1); // 50 × 0.02
    expect(r.resolve('grande', env).beadSec).toBe(0.2); // 10 × 0.02
  });
  ...
  it('usa o hop_sec do próprio envelope', () => {
    ...
    expect(r.resolve('media', env).beadSec).toBeCloseTo(1, 10); // 25 × 0.04
  });

  it('sem acousteme, cai na grade uniforme fixa do tokenizador (0.20 / 0.50 / 1.00 s)', () => {
    const r = new AcoustemeGranularityResolver();

    expect(r.resolve('pequena', null).beadSec).toBe(0.2);
    expect(r.resolve('media', null).beadSec).toBe(0.5);
    expect(r.resolve('grande', null).beadSec).toBe(1);
  });
