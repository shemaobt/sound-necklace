...
- @/adapters/voice/register.ts default-exports `{ port: 'voice', fixture, real }`; the composition root @/ui/app picks it up via `import.meta.glob('/adapters/*/register.ts')` (@/docs/architecture.md §4). Fixture is the default; the real mode activates later by environment config (ENG-247).
