...
- **`noteSessionCreated` is a no-op in real mode.** The API's `create_session` stamps the resolved `bead_sec` in the same transaction that writes the session — and writes the level too when the project has no row, which is how a project that predates this table gets frozen. It is on the port only because the fixture has no server to do that for it.
- The fixture's registration **seeds the fixture project (`projeto`) at `medium`**, which under lock-on-confirm means the demo opens already confirmed — like a real project in use. Without the seed the demo's Setup could not create a session at all until somebody visited Configurações. To exercise the unconfirmed screen, construct a bare `FixtureProjectSettings()`.
