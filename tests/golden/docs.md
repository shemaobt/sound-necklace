...
- **Session-step replayer** (`replaySessionSteps`, same file, ENG-216): replays the session/scene step prefix (segment → confirmWhole → cutScene → reopenScene → confirmParts) through the pure reducers of @/domain (`createSession`, `confirmWhole`, `confirmPart`, `reopenPart`, `confirmParts`). The `cutScene` step mirrors the reference driver in @/tests/golden/generate.mjs: it simulates the second click by setting the selection to `{frontier, endBead}` and confirming the current scene. Steps from later issues (triage, phraseSelect, answer, export, …) stop the replay and are reported in `pendingAt` — so `minimal-flow` stays PENDENTE in the harness until ENG-219+ register their steps and the real export lands (ENG-227/233). A domain-rejected step (a `SceneResult` error) throws loudly, so a case can never silently half-replay. Colocated tests (@/tests/golden/registry.test.ts) pin the scene-prefix replay of `minimal-flow`, including the reopen cascade and `PT#` stability.
