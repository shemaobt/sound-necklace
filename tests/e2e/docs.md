...
- Two resilience proof gotchas for future authors: the confirm controls (`✓ Confirmar esta cena/frase`) render whenever an anchor is active, so **"no confirm button" is not a blocked-editing proof** — assert no locked chip is born and the persisted DTO is deep-equal unchanged instead; and `data-play` lighting is **cumulative**, so proving playback *continues* while offline tracks the moving `[data-play="head"]` playhead index (a `.first()` match or a static "still lit" check cannot tell playing from frozen).
