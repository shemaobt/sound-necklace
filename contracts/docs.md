...
- The first real modules landed with ENG-227: @/contracts/serialize.ts (the one serializer + export filenames), @/contracts/manifesto.ts and @/contracts/retorno.ts (schema + mapper + export gate each), plus valid/invalid schema fixtures under `fixtures/`. ENG-235 added @/contracts/api.ts and @/contracts/bucket.ts (**provisional** endpoint + bucket DTOs). Remaining modules arrive via ENG-233 (report `.md` builder) and ENG-234 (session state + imports).
