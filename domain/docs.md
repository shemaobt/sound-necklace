...
- One deliberate, documented deviation exists: `buildBeads` throws on `beadSec <= 0` (and NaN). The reference performs this validation in its UI caller (`segment()`); in the pure domain the guard moved into the function per the ENG-214 DoD. The reference never calls with an invalid value, so byte-identity is unaffected.
