...
## ENG-258 — scan de minimalismo do ouvinte (§9.2) + mapa de bloqueios do E6 (verificado 2026-07-11)
...
- **Regra 2 usa o MARCADOR semântico, não o variante visual.** O `primary` da telha é reusado além da ação
  dominante (na Triagem convivem "▶ Ouvir esta cena" [primary] E o "Confirmar" da confiança [primary default]);
  contar `data-variant="primary"` marcaria UI legítima. A única ação dominante que as estações declaram é
  `[data-role="primary-action"]` (os testes por página já a fixam em 1; Mapeamento tem 0 — o mic não é marcado,
  daí o scan usa ≤1, não ==1).
- **Raiz do scan = `main.cds-app-main`** (só a estação; header + fio-de-contas global + diálogos portalizados
  ficam FORA). O **seam-modal** e a **gaveta de cobertura** são Radix `Dialog.Portal` → renderizam em
  `document.body`, fora de `main`: o seam-modal é escaneado à parte via `.cds-seam-modal`; a cobertura é
  facilitadora e fica FECHADA por padrão (conteúdo com dígitos/códigos nem entra no DOM) → não precisa excluir.
- **Por que as telas passam digit-free:** `sceneLabel`/`phraseLabel` soletram ("Cena um"/"Frase um",
  `ui/pages/escuta2/cutting.ts` UNIDADES/DEZENAS); `KindCard` mostra o rótulo PT e põe o código inglês só no
  `title` (hover, não é `innerText`); `ProgressDots` são só aria-label; confiança = palavras (certeza/quase/na
  dúvida); aviso de cena vazia = "Esta cena ficou sem frases. Clique de novo para seguir mesmo assim." (sem dígito).
- **Estados alcançados num único percurso** (o roteiro do acceptance-1): Escuta 1 → Escuta 2 (ancoragem + cena
  travada com chips) → Triagem (foco/"Ver todos os tipos por tema"/passo de confiança) → Segmentação (ancoragem/
  "Pronto com esta cena →" sem frases = aviso vazio/seam-modal ao confirmar frase que cruza a borda) → Mapeamento
  (L1 história, L2 cena, L3 frase por detecção do ▶, + estado "gravando" após "gravar a resposta"). Prova de
  mordida (throwaway) confirmou que cada regra falha em violação injetada — o scan não é no-op.

- **MAPA DE BLOQUEIOS DO E6 (todos por falta de wiring do app ao boundary E2E — testes read-only não podem fechar):**
  - **255** (oral-mode) → BLOCKED, desbloqueador **ENG-275** (Player de áudio nunca é injetado no runtime;
    `App.tsx` `player = {stop(){}}` no-op → sem "som antes do texto").
  - **256** (interview completeness) → BLOCKED, desbloqueador **ENG-276** (voz gravada só vai ao `MemoryVoiceStore`
    em memória; `meta.voice` nunca é atualizado → Export monta o `.md` com `custody.voice=new Set(dto.voice)`=∅ →
    respostas de voz viram "(sem resposta)"; a prévia do relatório no Mapeamento renderiza `<RelatorioStation/>`
    SEM `recorder`). Prova empírica: 3 respostas por voz + conclusão → `.md` guardado com **zero** `respostas/`.
    O blob EM SI grava no caminho exato (revisitar a pergunta re-mostra "ouvir" → `recorder.has(path)`=true).
  - **257** (resilience: drop/re-login/lock) → BLOCKED de fato: `useOnline` cria o `FixtureConnectivityMonitor`
    internamente, sem hook de janela, e o monitor fixture IGNORA os eventos `online`/`offline` do browser (só o
    `browser.ts`, que não é o default, os escuta) → um E2E não tem como alternar conectividade/expiração de auth.
  - **258** (este) → o ÚNICO E6 sem dependência de wiring (scan de DOM puro) → entregue.
