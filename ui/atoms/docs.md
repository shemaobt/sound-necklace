...
- **Accessibility split:** decorative atoms (Pearl, CordLine, WaveformBar, PlayGlyph) are `aria-hidden` — the interactive parent (a molecule's row or button) carries the accessible name. Interactive atoms are real `<button>`s: Chip exposes `aria-pressed` mirroring `data-selected`. ConfidenceDisc is dual-mode: with a `label` prop it renders `role="img"` + `aria-label`; without one it is `aria-hidden`.
