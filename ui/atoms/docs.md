...
- **Those fallbacks are where the theme enters Pearl, and only there (ENG-391).** The untinted pearl now falls back to `--cds-ui-pearl` / `--cds-ui-pearl-lit`, which are the pérola-aveia tokens in light and translucent cream in dark — the "unDark" treatment from `docs/design/pacote-melhorias-ui.md` §8, so an untouched bead reads as a soft pearl instead of punching a hole of light in a dark page. A **tinted** pearl never goes through the theme: the molecule injects the scene's own triple, so the same scene keeps the same colour in both themes (a scene's colour is data, not chrome). The scene-end square additionally carries `--cds-ui-bead-ring`, which is `transparent` in light (drawing nothing, so light is untouched) and cream at 30% in dark.
...
- Pearl's scene-end variant (`data-scene-end`) renders square-ish with a flat deep fill; a sibling `phraseEnd` prop (ENG-725, `data-phrase-end`) marks a phrase's last bead instead — a smaller rounded square, deep tone, `scale(0.86)`, and no ring, distinguishing it from the larger, ringed scene-end square. **Precedence lives in the atom, not the caller:** `data-phrase-end` is only ever emitted when `!sceneEnd`, so a bead that closes both a phrase and a scene renders as the scene-end mark alone. Button's primary hover/active/focus-visible all darken to `--cds-telha-deep` — press darkens, never lightens (redesign §4.1). Button carries two distinct unavailable states: `disabled` (the native attribute — the button is out of reach and silent) and `ariaDisabled` (ENG-393 — dimmed and announced unavailable, but still clickable so the caller can answer the tap; on ear-first screens that answer is a refusal sound, and a natively disabled button would swallow it). PlayGlyph is inline SVG (viewBox 24, `currentColor`), never a unicode character.
- `Pearl` also takes `noneFit` (ENG-725): a bead of a none-fit scene renders cream with a dashed ring, and at the scene end it stays cream and dashed — square like every scene end, but without the deep tone or the ring (`data-none-fit`, design `none`/`noneEnd`).
- The prototype-divergence this used to record is resolved: @/ui/tokens now ships the prototypes' hand-tuned per-hue `{base, lit, deep}` triples verbatim (no more `lit = base` / a formula-derived `deep`) — see @/ui/tokens/README.md. Atoms simply render whatever `PaletteEntry` they are handed, so that fix required no atom change.
