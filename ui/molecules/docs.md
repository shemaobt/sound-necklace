...
- Molecules carry no domain meaning. They emit **presentational tokens**, not domain enums: ConfidenceTrio yields `'certeza' | 'quase' | 'duvida'` and the calling organism maps those to the domain's `alta`/`média`/`baixa` (the map lives in @/ui/organisms/triagem-picker). Bead/scene geometry (how many beads land on each row, which are edge beads) is computed above by the necklace organism and handed in as props.
- Feeds the interaction-critical organisms that compose these pieces — e.g. the necklace (BeadRow + SelectionBand), the Triagem picker (KindCard + ConfidenceTrio) and the conversation stage (QuestionCard) — plus the dashboard (ENG-222) and the app shell (StepperStation); see @/ui/organisms/docs.md and @/ui/docs.md for the layers above. (Not every organism needs a molecule: the seam modal composes atoms directly.)
...
- **Accessibility structure is the molecule's job** and follows ARIA APG / WCAG:
  - Single-select gestures are `radiogroup`/`radio` with `aria-checked` (not toggle buttons): ConfidenceTrio owns a full roving-tabindex group (one tab stop, arrow/Home/End move-and-select); KindCard is an isolated `role="radio"` whose group and roving tabindex are orchestrated by the Triagem picker organism — KindCard forwards no ref, so the organism roves focus by querying `[role="radio"]` on its container (see @/ui/organisms/docs.md).
...
- **Two molecules were realigned to the Protótipo** (`docs/design/Colar de Sons - Protótipo.dc.html`, CLAUDE.md precedence rule 2):
  - `StepperStation` renders an 11px (current) / 7px (done/future) dot instead of the previous 20px/13px pearl; the current dot's `PEARL_STATE` maps to `'lit'` (a color) rather than `'head'` (the playhead ring) — its emphasis now comes from size + a CSS halo, not from the ring style. The visible station NAME moved out of this molecule into @/ui/app/stepper.tsx, which renders it once above the `<ol>` for the CURRENT station only; the sr-only label here is unchanged.
  - `ProgressDots` gained an optional `scenes: ProgressDotScene[]` prop (`{ state: 'tagged' | 'none_fit' | 'pending', tint? }`) so the Triagem station (@/ui/pages/triagem) can render each dot as: a scene-tinted pearl with a white check (`tagged`), the same tint desaturated with no check (`none_fit`), or a hollow ring (`pending`, the default when `scenes` is omitted). The check is an inline SVG, not a glyph component.
