...
- The second presentational layer of the Shemá design system (ENG-218): reusable compositions of @/ui/atoms — a bead row over a cord, the pending selection band, the footer bead strip of saved scenes/phrases, the confidence-trio gesture, the scene-kind card, the Conversation question card, the Export document card, the "fio de contas" stepper station, Triage progress dots, the Setup trust chip, the autosave save-chip, the conversation per-trecho progress bar and the trecho indicator (colour dot + trecho name beside the ▶).
...
- **Accessibility structure is the molecule's job** and follows ARIA APG / WCAG:
  ...
  - BeadStrip is the footer's bead cord (ENG-388): a `role="group"` of bead `<button>`s (each an @/ui/atoms `Pearl` behind an `aria-label`, `aria-pressed` on the chosen one) plus **at most one** capsule carrying that item's `actions` slot. Controlled and stateless — `selected` in, `onSelect(key)` out — and a `selected` that is no longer in `items` renders no capsule, so a stale key can never offer actions on a removed item. The action set is the caller's and did **not** change: the design delivery's play/flag/reopen buttons are demo filler that would silently reverse ENG-291 (sound comes only from necklace beads) and ENG-342 (a locked item is adjusted by dragging its end handle).
...
- **State via props, effects via slots.** DocumentCard toggles Baixar→baixado through a `downloaded` prop (`data-downloaded`); the actual Blob/anchor download is the caller's. QuestionCard exposes an answer slot (`children`) for the recorder/waveform; BeadStrip takes each item's `actions` as a slot and renders only the selected item's. Molecules never own IO.
