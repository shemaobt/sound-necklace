...
- **Question order is the domain's.** The flat card order comes verbatim from `questionSequence` (@/domain/mapping.ts): L1 `L1_Q` → per `lockedParts` (none_fit included) `L2_Q` → per `productiveFrases` `L3_Q`. The UI only inserts section (level) and scene/phrase group headers as it walks the sequence.
...
- **Voice discovery.** One effect probes `recorder.has(path)` for every question path and stores the set of paths that HAVE a recording. That set both (a) drives the voice-only row and (b) feeds `buildMapReport`'s `voice` argument so the `.md` references the recording. With no recorder the set stays empty (no synchronous setState in the effect — `react-hooks/set-state-in-effect`).
...
- **Voice duration is a documented gap.** The voice row renders a duration slot (`aria-label="duração da resposta"`) but shows a placeholder `—`: the merged `VoiceRecorder` port (@/adapters/voice/types.ts) exposes duration only on a fresh `stop()` (`RecordedAnswer.durationSec`), not for an already-stored answer, and the adapter is out of this station's scope. A follow-up should add `duration(path)` to the port (or persist the duration at record time in Mapeamento) so the row shows the real length.
