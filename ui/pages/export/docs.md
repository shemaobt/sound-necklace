...
- **The report gate is new and is NOT from the reference** (ENG-327). `reportExportStatus(session, custody.voice)` (@/contracts/relatorio.ts) is consulted with the session's recorded-answer paths; while any recorded answer lacks confirmed text it blocks BOTH the report download and "Concluir e guardar", showing the `export.reportBlocked` notice and playing the refuse sound. The reasoning: since the recording never enters the `.md`, saving such an answer would lose what the person said, silently. The way out is confirming the English draft in @/ui/pages/report — or typing the answer by hand there, which always works.
