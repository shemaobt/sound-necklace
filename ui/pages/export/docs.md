...
- A wiring-layer page: it reads the pure @/domain `session` from the @/ui/state store and receives the `SessionStore` adapter + session id **by prop** (there is no shell mechanism yet to hand a page the sessions adapter or the current session id).
...
- **Not yet reachable in the running app.** Wiring the station into the shell (@/ui/app/App.tsx `KEY_TO_MODE`, stepper `reachable`) and passing it `store`/`sessionId` was **out of scope for ENG-246** and is a follow-up. In the app the station renders `edit` phase with no store; it is fully covered by its jsdom tests, which inject a `FixtureSessionStore`.
...
- **Phase state machine** `loading | edit | saved`: on mount (only when `store`+`sessionId` are present) it reads `store.get(id).status` and `store.load(id)`. `load` recovers the meta (granularity/bucket/voice/consent) from the persisted DTO because the @/ui/state store holds only the domain state, not the meta; `toSessionDto` needs it. `concluida → saved` (review), else `edit`; a never-saved DTO falls back to `DEFAULT_META`.
...
- **Gates mirror the reference** (docs/reference/index.html): the retorno download requires `retornoExportStatus(session).canExport` (whole confirmed) else it shows the exact copy "Confirme o colar antes de exportar." and does not download; the manifesto download silently no-ops unless `canExportManifesto(session)` (totalBeads > 0).
...
- **No browser test.** The `Necklace` is shown read-only (`transportOnly`) and its geometry is browser-tested in its own organism; the jsdom suite (`export.test.tsx`) covers this station fully. The cream stage (@/ui/pages/export/export.css) has no decorative motion — the only celebration is the organism's chip under its own reduced-motion guard.
- **Known follow-ups:** (1) shell wiring to make the station reachable and pass `store`/`sessionId`; (2) the `ArtifactCards` organism chip still carries the v1 local-custody wording ("documentos salvos — nada saiu deste computador") whereas PRD v2 §5 is cloud custody — changing the organism was out of this issue's scope.
