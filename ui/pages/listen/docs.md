...
- Reads session state via `useSessionStore` and writes only through `sessionStore.apply` (see @/ui/state/docs.md), so the store's editability gates (online/review/lock) can silently pause the confirm without losing in-memory state.
- Confirm dispatches `confirmWhole` (@/domain/scenes.ts), which advances the guided flow — it sets `whole.confirmed`, flips `mode` to `escuta`, drops `review`, and enters the parts (scenes) layer. Once confirmed, Escuta 2 (Cortar) takes over; the Escuta screen shows no action (ENG-342 removed the "Reabrir"/`reopenWhole` — un-confirming the story lives in Cortar's "← Voltar").
...
- **The same heard-bead set feeds the whole-story bar in the shell** (ENG-648): on every newly visited bead the station publishes `heard.size` to @/ui/state's `progressStore`, which is where @/ui/app/story-progress.tsx reads the Ouvir fraction from. Cumulative COVERAGE, not the playhead — the v4 prototype uses `(heardTo + 1) / N`, a playhead that walks backwards when playback restarts, and a whole-story bar that retreats reads as lost work. The station keeps `heard` as a ref, so this is the only thing that makes the number visible outside it; nothing about it enters the session state or any artifact.
...
- **Confirm** — `confirmWhole` returns a result; on failure the exact domain error copy is shown in a `role="alert"` line, on success `apply(() => result.state)` replaces the session.
