...
- **Verbatim reference copy:** delivery success is `✓ Entrega carregada: N cena(s), M frase(s). …`; return success is `✓ Retomado: N cena(s), M frase(s).`. Delivery additionally pushes the `MANIFEST_MISMATCH_MSG` warning when the mapper reports `manifestMismatch` **and still loads**. Return has no mismatch warning (mirrors the reference — return import never warns on manifest).
