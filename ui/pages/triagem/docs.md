...
- **Progress dots** (@/ui/molecules `ProgressDots`): one per locked part, `current = focusIdx`, `onSelect = setFocusIdx`. Digit-free (`aria-label="ir para a cena"`, `aria-current="step"` on the focused dot).
...
- **Current tag state** (`tagShow`) is always visible for the focused scene: `⌀ nenhum se encaixa` / `<skShort(kind)> · <confLabel>` (certeza/quase/na dúvida, from reference L1211) / `— por classificar`.
- **The gate** ("Já classifiquei todas as cenas →", dark, `data-role="primary-action"`) is disabled unless `triagemDone(state).enabled` (all locked parts non-pending **and** ≥1 productive); its helper message comes verbatim from `triagemDone(state).message`. On advance it composes the pure `setMode(state, 'segmentacao')` with `enterSegmentacao` **only when the effective mode is segmentacao** (guarded by the enabled gate), a faithful port of the reference's setMode layer-entry (index.html L1006–1008).
...
- **Progress-dot check marks omitted:** the design shows a check inside classified dots, but `ProgressDots` has no "done" prop and the molecule is out of this issue's scope; the always-visible tag state gives the same per-scene feedback.
