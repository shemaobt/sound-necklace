...
- **The gate** is `triagemDone(state)` (all locked parts non-pending **and** ≥1 productive); it no longer renders its own disabled button — it only drives `reviewing` (see above) and, while NOT enabled, shows `gate.message` verbatim as a `role="status"` guidance line. That message is @/domain copy and therefore **stays PT-BR even with the UI in EN** (translating it would touch a frozen layer; see @/ui/i18n/docs.md). `advance` (now reached only through the review `review.continue` button) composes the pure `setMode(state, 'segmentacao')` with `enterSegmentacao` **only when the effective mode is segmentacao** (guarded by the enabled gate), a faithful port of the reference's setMode layer-entry (index.html L1006–1008).
