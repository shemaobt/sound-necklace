...
- **Reconciliations prototype ↔ contract** (CLAUDE.md precedence rule 1 — data wins over the prototype's look, whose source is @/docs/design/README.md): the field stays "Usuário"/`username` (the auth contract keys on `username`; fixture/e2e use `facilitadora`/`admin`) rather than the prototype's "E-mail"; the prototype's `noop` "Criar conta"/"Esqueceu a senha?" links are omitted (no MVP flow); the brand mark is the inline `ColarIcon` because there is no svg-asset pipeline — not for the prototype's `logo-branco.svg` wordmark, not for `pattern-tile.svg`, and not for `docs/design/website-icon.svg` either, which is why that mark is ported as inline JSX rather than imported. Focus outline and `prefers-reduced-motion` are inherited from the global @/ui/tokens/base.css floor — no per-file duplication.
