...
- The success/error branch mirrors the domain contract exactly: the "span incomplete" copy comes from @/domain (`SCENE_ERROR_COPY.WHOLE_SPAN_INCOMPLETE`), never re-authored in the UI, so the guided flow never advances unless the whole span covers 0…N−1.
