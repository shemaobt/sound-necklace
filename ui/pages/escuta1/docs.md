...
- **First occupant of the previously-empty @/ui/pages stations layer.** Stations self-register through the `import.meta.glob('/ui/pages/*/index.tsx')` station registry built in @/ui/app/registries.ts, so the default export in `index.tsx` is mandatory — it is the value the registry stores. The directory name `escuta1` is the registry key, and @/ui/app/App.tsx's `KEY_TO_MODE` maps it (alongside `escuta2`) to the domain `escuta` mode.
...
- Decorative tagline motion (@/ui/pages/escuta1/escuta1.css) is guarded by `@media (prefers-reduced-motion: no-preference)`; bead lighting motion lives in the atoms, already guarded there.
- Tests split by suffix: `transport.test.ts` and `escuta1.test.tsx` run in jsdom; `escuta1.browser.test.tsx` runs in real Chromium for bead/head taps against real geometry (jsdom has no Web Audio and zeroed layout).
