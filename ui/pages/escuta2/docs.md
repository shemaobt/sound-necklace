...
- **Header + white pending-selection play** (realigned to the Protótipo): the instruction now sits under a `title` heading ("Corte a história em cenas", @/ui/i18n `escuta2.title`); the necklace uses the shared `SIZE_L` preset (26px beads, same as @/ui/pages/escuta1). A `playSelection` handler (a white ghost button, `escuta2.playSelection`) plays the pending (unconfirmed) selection via `player.toggle('selecao-pendente', ...)` — distinct from the per-scene `▶ ouvir` on a locked chip.
...
- **The review moment (`tiled`, owner decision):** once the locked scenes cover the whole story (the highest locked scene's end bead equals `totalBeads - 1`), the residual anchor the domain still opens (`confirmParts` just discards it, PRD §8.4) is hidden and the station collapses to one headline (`escuta2.reviewHeadline`) plus one action (`review.continue`, shared with @/ui/pages/triagem and @/ui/pages/segmentacao) that dispatches the same `confirmParts`. Partial coverage (scenes still short of the story end) keeps the PRD's own instruction/play/confirm-scene/"Confirmar as cenas →" flow untouched — the PRD's CTAs are never removed, only superseded once nothing is left to cut.
