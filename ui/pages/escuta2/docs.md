...
- The two confirm-scene error copies come verbatim from the domain contract `SCENE_ERROR_COPY` — "Clique onde a cena termina, no colar." (`SELECTION_INCOMPLETE`) and "A cena não pode começar antes da conta X." (`SCENE_BEFORE_FRONTIER`) — never re-authored in the UI; shown in a `role="alert"` line.
