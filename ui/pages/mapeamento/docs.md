...
- **`Mapeamento({ player, recorder, onVoiceSaved })`** (@/ui/pages/mapeamento/index.tsx): subscribes `session`; renders `null` until a session with a non-empty question sequence exists. `onVoiceSaved?` is the optional shell callback (ENG-276) invoked with the canonical voice path after a recording is persisted, so the shell can append it to `meta.voice` (see @/ui/app/docs.md). Navigation is **screen scaffolding** — the domain does not persist `mapStep` (see @/domain/state.ts) — so the current question is a local index and the report is a local `atReport` flag.
...
- **The audio/voice seam is a prop seam.** `player` and `recorder` default to `null` and `onVoiceSaved` is optional; with none, the page renders and navigates but playback/recording are dormant (same convention as the sibling stations). The shell now injects all three when Mapeamento is current — the fixture recorder (ENG-273), the per-session `Player` (ENG-275) and `onVoiceSaved` (ENG-276, threads the voice path into `meta.voice`) — see @/ui/app/docs.md. Binding the recorder to the active session's SessionStore resources remains a documented follow-up (@/adapters/voice/register.ts).
