...
- The third presentational layer of the Shemá design system: stateful compositions that consume @/domain state via props/hooks and own the interaction-critical surfaces the listener touches. This folder ships the **necklace**, the **conversation stage** and the **storyteller guide**, all exported from @/ui/organisms/index.ts.
...
- **The recorder-flow interaction test is mandatory** and lives in `conversation-stage.browser.test.tsx` (real Chromium, raw `createRoot`/`flushSync` + native click), because the conversation recorder is one of CLAUDE.md's three interaction-critical surfaces; a companion `conversation-stage.test.tsx` runs in jsdom for the structural assertions.
- **prefers-reduced-motion is honored throughout**: the only decorative animation (the idle mic pulse in the conversation stage) is gated under `@media (prefers-reduced-motion: no-preference)`, and the static storyteller guide carries no motion at all.
