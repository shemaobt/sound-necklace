 ...
 * personagem ilustrado) → `animated` (ENG-232, SVG animado em CSS) → `static`. Mecanismo
 * aditivo do doc de arquitetura §4 — igual às três registries do shell.
 *
...
export function pickVariantPath(paths: string[]): string {
  const chosen =
    paths.find((p) => /\/lottie\.tsx$/.test(p)) ??
    paths.find((p) => /\/animated\.tsx$/.test(p)) ??
    paths.find((p) => /\/static\.tsx$/.test(p));
