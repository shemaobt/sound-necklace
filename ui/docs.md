...
- Present so far: @/ui/tokens (Shemá tokens), @/ui/atoms (the presentational base layer — see @/ui/atoms/docs.md) and @/ui/molecules (reusable atom compositions — see @/ui/molecules/docs.md); @/ui/app/App.tsx is a placeholder replaced by the real shell in ENG-224 (routes, "fio de contas" stepper, itinerant player, review/lock, connection gate), plus a browser-mode smoke test proving the real-Chromium test pipeline. Organisms (necklace, Triagem picker, seam modal, conversation stage, dashboard) and the app shell are the next consumers.
