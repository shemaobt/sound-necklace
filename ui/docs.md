...
- Currently a minimal shell: @/ui/app/App.tsx is a placeholder replaced by the real shell in ENG-224 (routes, "fio de contas" stepper, itinerant player, review/lock, connection gate), plus a browser-mode smoke test proving the real-Chromium test pipeline.
- Design/copy source: the Claude Design prototypes win for look/layout/motion (@/docs/PRD-redesign.md); styling is CSS custom properties + CSS Modules — token values port 1:1 from the prototypes and are unit-testable.
