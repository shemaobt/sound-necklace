...
- The Shemá design system plus the flow stations (Setup, Escutas, Triagem, Segmentação, Mapeamento, Export, Dashboard, Login), organized by **atomic design**: `tokens → atoms → molecules → organisms → templates/pages`, with `app` as composition root, `state` as the domain-state bridge and `i18n` as the language layer.
- Present so far: @/ui/tokens (Shemá tokens), @/ui/atoms (the presentational base layer — see @/ui/atoms/docs.md), @/ui/molecules (reusable atom compositions — see @/ui/molecules/docs.md) and @/ui/organisms (the stateful, interaction-critical compositions — see @/ui/organisms/docs.md), home of the hero necklace, the conversation stage, the seam modal (the border-crossing dialog), the shell-mounted tutorial popup, the Triagem picker and the coverage drawer — the dialogs/popup follow the repo's Radix policy (Radix primitives for behavior only, visuals 100% Shemá tokens). @/ui/app is the **composition-root shell** (ENG-224 — see @/ui/app/docs.md): History-API router, "fio de contas" stepper, the single itinerant player, review/lock chrome, the online-only connection gate, and the three add-a-file registries. @/ui/state is the **domain-state bridge** (see @/ui/state/docs.md). @/ui/i18n is the **language layer** (ENG-279 — see @/ui/i18n/docs.md): PT-BR default + EN for the UI chrome. @/ui/pages holds every wired flow station (one directory per station, auto-discovered by the shell's glob registry — each has its own docs.md).
...
- **Test split by file suffix** (@/vitest.config.ts): `*.test.tsx` runs in the jsdom `dom` project; `*.browser.test.tsx` runs in real Chromium (`browser` project, `pnpm test:browser`). jsdom has no Web Audio, no MediaRecorder, and zeroed layout — so the interaction-critical organisms (necklace, seam modal, conversation recorder) **must** be tested in the browser project. The smoke test at @/ui/browser-smoke.browser.test.tsx proves the pipeline. Both projects load @/ui/i18n/test-setup.ts as a `setupFiles` entry — without it `t()` returns raw keys and every PT-BR copy assertion fails.
