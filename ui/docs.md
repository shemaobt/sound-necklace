...
- Present so far: @/ui/tokens (Shemá tokens), @/ui/atoms (the presentational base layer — see @/ui/atoms/docs.md), @/ui/molecules (reusable atom compositions — see @/ui/molecules/docs.md) and @/ui/organisms (the stateful, interaction-critical compositions — see @/ui/organisms/docs.md), home of the hero necklace, the conversation stage and the seam modal (the border-crossing dialog, first Radix-behavior + Shemá-visuals component). @/ui/app is now the **real composition-root shell** (ENG-224 — see @/ui/app/docs.md): History-API router, "fio de contas" stepper, the single itinerant player, review/lock chrome, the online-only connection gate, and the three add-a-file registries. @/ui/state is the **domain-state bridge** (see @/ui/state/docs.md). @/ui/pages holds the flow stations, starting with the two Escutas — the ceremonial opening and the scene-cutting station (see @/ui/pages/escuta1/docs.md and @/ui/pages/escuta2/docs.md); the remaining stations (Triagem, Segmentação, dashboard) are the next consumers.
