...
- Present so far: @/ui/tokens (Shemá tokens), @/ui/atoms (the presentational base layer — see @/ui/atoms/docs.md), @/ui/molecules (reusable atom compositions — see @/ui/molecules/docs.md) and @/ui/organisms (the stateful, interaction-critical compositions — see @/ui/organisms/docs.md), whose first member is the hero necklace (ENG-220). @/ui/app is now the **real composition-root shell** (ENG-224 — see @/ui/app/docs.md): History-API router, "fio de contas" stepper, the single itinerant player, review/lock chrome, the online-only connection gate, and the three add-a-file registries. @/ui/state is the **domain-state bridge** (see @/ui/state/docs.md). @/ui/pages holds the flow stations, whose first member is the ceremonial "Escuta 1" opening (ENG-229 — see @/ui/pages/escuta1/docs.md); the remaining stations and organisms (Triagem picker, seam modal, dashboard) are the next consumers.
