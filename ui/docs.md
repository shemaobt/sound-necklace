...
- Present so far: @/ui/tokens (Shemá tokens), @/ui/atoms (the presentational base layer — see @/ui/atoms/docs.md), @/ui/molecules (reusable atom compositions — see @/ui/molecules/docs.md) and @/ui/organisms (the stateful, interaction-critical compositions — see @/ui/organisms/docs.md), whose first member is the hero necklace (ENG-220); @/ui/app/App.tsx is a placeholder replaced by the real shell in ENG-224 (routes, "fio de contas" stepper, itinerant player, review/lock, connection gate), plus a browser-mode smoke test proving the real-Chromium test pipeline. The remaining organisms (Triagem picker, seam modal, conversation stage, dashboard) and the app shell are the next consumers.
