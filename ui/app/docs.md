...
- **Station host** (@/ui/app/station-host.tsx): resolves the active station from the registry; a not-yet-built station falls back to a quiet "estação em construção" line, so a station issue enters purely by adding its `ui/pages/<station>/` folder. It accepts optional `stationProps` and forwards them to the resolved component: most stations resolve everything internally (session via `useSessionStore`) and take no props, while stations needing shell-held wiring ports receive them here. Prop-less stations ignore unknown props. @/ui/app/App.tsx passes `{ store, sessionId }` only when the Export station is current — `store` is the app-global sessions adapter (@/ui/app/session-adapter.ts), `sessionId` comes from the `/session/:id` route — and holds a local `viewingExport` flag: clicking the reachable Guardar bead sets it true (landing on Export); navigating any other station clears it and dispatches `setMode` as before.
